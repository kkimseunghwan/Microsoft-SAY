...
f = open("C:\\Hwan\\workspace\\TestData\\SeoulSubwayData.csv", "r", encoding="utf-8")
for line in f.readlines():
    line = line.replace("\n", "").split(",")

    when = datetime.strptime(line[0] + "-" + line[1] + "-" + line[2], "%Y-%m-%d")
...
f = open("C:\\Hwan\\workspace\\TestData\\SeoulSubwayResult.csv", "a", encoding="utf-8")
weekData = []
for yoil, sum in subwaySum.items():
    f.write("%s,%.3ff\n" % (yoil, sum/subwayCnt[yoil]))
    print("%s 평균 : %d" % (yoil, sum/subwayCnt[yoil]))
    weekData.append([yoil, sum/subwayCnt[yoil]])

weekData.sort(key=lambda x: -x[1])
print(weekData)
